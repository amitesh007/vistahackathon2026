<#
.SYNOPSIS
    Locates the "Delete" workflow sheet in an extracted .xlsx workbook and
    returns key metadata needed to drive Delete API code generation.

.DESCRIPTION
    Reads xl/workbook.xml and xl/_rels/workbook.xml.rels to find the sheet
    whose name matches "delete" (case-insensitive).

    GUARDRAIL: If no "Delete" tab exists the script exits with code 2.
...
.OUTPUTS
    PSCustomObject with:
        SheetFile        - XML filename, e.g. "sheet7.xml"
        SheetName        - Original tab name, e.g. "Delete"
    ...
    Exit code 2 = guardrail failure (no Delete tab found).

.EXAMPLE
    $meta = .\find-delete-workflow-sheet.ps1 -ExtractedPath "C:\Auto\API\Principal_Payment_API_v1.10_extracted"
...
# GUARDRAIL: Find the Delete sheet (case-insensitive)
$targetSheetNode = $null
foreach ($s in $sheetNodes) {
    if ($s.GetAttribute('name') -imatch '^delete$') {
        $targetSheetNode = $s
        break
    }
}
...
if ($targetSheetNode -eq $null) {
    Write-Error 'GUARDRAIL FAIL: No tab named "Delete" found in workbook.'
    Write-Error 'Skill execution halted - a Delete tab is required to proceed with Delete API generation.'
    exit 2
}
...
Write-Host "Delete sheet found: '$sheetTabName'  rId=$sheetRId"
...
# Derive entity name: strip "Delete" prefix and "Integration" suffix
...
$entityName = $integrationClass -replace '^Delete', '' -replace 'Integration$', ''
Write-Host ''
