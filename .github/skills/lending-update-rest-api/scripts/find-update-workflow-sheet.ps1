<#
.SYNOPSIS
    Locates the "Update" workflow sheet in an extracted .xlsx workbook and
    returns key metadata needed to drive Update API code generation.

.DESCRIPTION
    Reads xl/workbook.xml and xl/_rels/workbook.xml.rels to find the sheet
    whose name matches "update" (case-insensitive).

    GUARDRAIL: If no "Update" tab exists the script exits with code 2.
...
.OUTPUTS
    PSCustomObject with:
        SheetFile        - XML filename, e.g. "sheet5.xml"
        SheetName        - Original tab name, e.g. "Update"
    ...
    Exit code 2 = guardrail failure (no Update tab found).

.EXAMPLE
    $meta = .\find-update-workflow-sheet.ps1 -ExtractedPath "C:\Auto\API\Principal_Payment_API_v1.10_extracted"
...
# GUARDRAIL: Find the Update sheet (case-insensitive)
$targetSheetNode = $null
foreach ($s in $sheetNodes) {
    if ($s.GetAttribute('name') -imatch '^update$') {
        $targetSheetNode = $s
        break
    }
}
...
if ($targetSheetNode -eq $null) {
    Write-Error 'GUARDRAIL FAIL: No tab named "Update" found in workbook.'
    Write-Error 'Skill execution halted - an Update tab is required to proceed with Update API generation.'
    exit 2
}
...
Write-Host "Update sheet found: '$sheetTabName'  rId=$sheetRId"

# Resolve rId to worksheet XML filename
...
# Derive entity name: strip "Update" prefix and "Integration" suffix
...
$entityName = $integrationClass -replace '^Update', '' -replace 'Integration$', ''
Write-Host ''
